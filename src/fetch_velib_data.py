...
def fetch_station_info(folder_path):
    ...
    if response_info.status_code == 200:
        ...
        # logging.info(f"{len(df_station_info)} stations loaded (infos)")
        df_station_info = df_station_info[["stationCode", "name", "lat", "lon", "capacity"]]
        # logging.info(f"Saving station info in {folder_path}")
...
def fetch_station_status(folder_path):
    ...
    if response_status.status_code == 200:
        ...
        # dt = datetime.fromtimestamp(data_status['lastUpdatedOther'])
        ...
        # logging.info(f"{len(df_dispos)} stations loaded (status)")
        df_dispos = df_dispos[
            [
                "stationCode",
                "num_bikes_available",
    ...
    # --- Extract Bike Counts ---
    # logging.info("Extracting bike counts")
    ...
    df_dispos = df_dispos.rename(columns={"is_returning": "is_working"})
